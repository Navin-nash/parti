...
- **One dark-mode switching mechanism.** `prefers-color-scheme` deciding some tokens while a `.dark` class decides others is a spec that disagrees with itself the moment a user's OS setting and the app's own toggle diverge. Pick one and route every themed token through it.
- **Declare the sRGB value first for anything in P3.** A wider-gamut color needs an sRGB fallback declared before the `@media (color-gamut: p3)` override, never the other way — browsers without P3 support need something to fall back *to*, not a hole.
- **Gradient interpolation space is part of the spec, not an implementation detail.** `in oklab` is the default — even brightness, no hue surprises. `in oklch` sweeps around the hue wheel and stays vivid; reach for it when a two-hue gradient goes gray in the middle. The unstated sRGB default darkens and mutes the midpoint, which is what ships when nobody chose.
